...
def normalize_chapter_order(value):
...
def chapter_list(request, law_id):
    ...
    chapters = ChapterModel.objects.filter(law=law).order_by('order')
...
def chapter_create(request, law_id):
    ...
    if request.method == "POST":
        order = normalize_chapter_order(request.POST.get("order"))
        chapter_number = request.POST.get("chapter_number")
        title = request.POST.get("title")
        description = request.POST.get("description")
        ChapterModel.objects.create(order=order, law=law, chapter_number=chapter_number, title=title, description=description)
...
def chapter_update(request, law_id, pk):
    ...
    if request.method == "POST":
        chapter.order = normalize_chapter_order(request.POST.get("order"))
